...
echo "===== TOTAL WALL-CLOCK (trigger -> idle) ====="
echo "[bench] total_wall_clock_s=${WALL}"

echo "===== PERF SUMMARY full_rebuild (stage breakdown) ====="
grep -E "PERF SUMMARY full_rebuild" "$LOG" | tail -1 || echo "(no full_rebuild PERF SUMMARY found)"
echo "===== PERF SUMMARY phase2 / stage3 ====="
grep -E "PERF SUMMARY (phase2|streaming_index stage3)" "$LOG" | tail -2 || true
...
echo "[bench] first_query_latency_ms=${QLAT}"
echo "[bench] first_query_response_head=${QRESP:0:160}"
...
echo "[bench] cold_first_query_latency_ms=${CQLAT}  (includes shard load_from_db warm)"
echo "[bench] cold_first_query_response_head=${CQRESP:0:160}"
...
echo "[bench] warm_second_query_latency_ms=${WQLAT}  (shard already resident)"
echo "===== TIME-TO-QUERY SUMMARY ====="
echo "[bench] index_wall_clock_s=${WALL}  cold_first_query_ms=${CQLAT}  warm_query_ms=${WQLAT}"
